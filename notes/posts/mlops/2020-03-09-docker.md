...
icon: docker.svg
good: true
keywords: "pybash tania rascia CI CD continuous integration deployment pipeline docker idea how to use airflow kubernetes k8s k apache container images dangling images vscode vsc visual studio code ssh container env environnement file variable container runtime and devel nvidia images"
date: 2022-10-01
---
...
If you use `docker images -a` and see a lot of `<none>:<none>` images. Don't be worry to fast, if they're dangling images, they take spaces, otherwise, they're harmless to your drive! Check [this SO](https://stackoverflow.com/a/53224187/1323473).
...
- Check [this SO](https://stackoverflow.com/a/21564990/1323473) for `CMD` vs `ENTRYPOINT`.
...
You cannot use something like `COPY .. .` (parent of the folder containing the `Dockerfile`) in the `Dockerfile` because it uses the "build context" and it can access to files within that context. [Ref](https://docs.docker.com/engine/reference/builder/#copy).
...
::: info
To upgrade docker-compose file format from version `2.x` to version `3.x`, check [this guide](https://docs.docker.com/compose/compose-file/compose-versioning/#upgrading). Also check this: [Compose file versions and upgrading](https://docs.docker.com/compose/compose-file/compose-versioning/).
:::
...
- [What is the difference between devel and runtime tag for a Docker container? - Stack Overflow](https://stackoverflow.com/questions/56405159/what-is-the-difference-between-devel-and-runtime-tag-for-a-docker-container)
